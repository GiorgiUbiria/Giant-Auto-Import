...
export const userPricingConfig = sqliteTable(
  "user_pricing_config",
  {
    id: integer("id", { mode: "number" }).primaryKey({ autoIncrement: true }),
    userId: text("user_id")
    ...
    oceanRates: text("ocean_rates", { mode: "json" })
      .$type<Array<{ state: string; shorthand: string; rate: number }>>()
      .notNull()
      .default(sql`'[]'`),
...
export const csvDataVersions = sqliteTable(
  "csv_data_versions",
  {
    id: integer("id", { mode: "number" }).primaryKey({ autoIncrement: true }),
...
export const oceanShippingRates = sqliteTable(
  "ocean_shipping_rates",
  {
    id: integer("id", { mode: "number" }).primaryKey({ autoIncrement: true }),
