...
import { carTable, priceTable, transactionTable, userTable } from "../drizzle/schema";
...
const insertTransaction = async (transaction: NewTransaction) => {
  return db.insert(transactionTable).values(transaction).returning({ transactionId: transactionTable.id });
};
...
export async function getTransaction(
  ...
  try {
    ...
    return { success: "Transaction retrieved successfully", error: null, data: singleTransaction };
  } catch (error) {
...
export async function addTransaction(
  ...
  try {
    ...
    if (price.currencyId !== priceCurrencyId) {
      try {
        finalAmount = await convertCurrency(amount, priceCurrencyId, price.currencyId!);
    ...
    return { success: "Transaction added successfully", error: null, data: null };
  } catch (error) {
    return { error: "Error adding transaction", data: null };
  }
}

async function convertCurrency(amount: number, fromCurrencyId: number, toCurrencyId: number): Promise<number> {
...
export async function deleteTransaction(
  ...
  try {
    ...
    if (transaction.currencyId !== price.currencyId) {
      try {
        rollbackAmount = await convertCurrency(transaction.amount!, transaction.currencyId!, price.currencyId!);
      } catch (conversionError) {
        return { error: "Currency conversion failed", data: null };
      }
    }
    ...
    return { success: "Transaction deleted successfully", error: null, data: null };
  } catch (error) {
    return { error: "Error deleting transaction", data: null };
  }
}
